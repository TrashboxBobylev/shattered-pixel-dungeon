...
import com.trashboxbobylev.tormentpixeldungeon.plants.Plant.Seed;
...
import com.trashboxbobylev.tormentpixeldungeon.ui.ItemSlot;
			...
			if (item != null) {
				...
				if (item.name() == null) {
					...
					enable(
						...
						mode == Mode.ALCHEMY && ((item instanceof Seed && !(item instanceof BlandfruitBush.Seed)) || (item instanceof Blandfruit && ((Blandfruit) item).potionAttrib == null) || item instanceof Potion) ||
						mode == Mode.ALL
					);
